...
namespace ProgrammingQuestions
{
    public class Lottery2
    {
        ...
        class LotteryEntry
        {
            ...
            public LotteryEntry(string spec)
            {
                string[] specs = spec.Split();

                Name = specs[0];
                Choices = int.Parse(specs[1]);
                Blanks = int.Parse(specs[2]);
                IsSorted = specs[3] == "T";
                IsUnique = specs[4] == "T";
            }

            public long CalculateNumberOfTickets() 
            {
                long combinations;
                if (IsUnique)
                {
                    combinations = UniqueCombinations(Choices, Blanks);
                }
                else
                {
            ...
            private long UniqueCombinations(long choices, long blanks)
            {
                long result = 1;
